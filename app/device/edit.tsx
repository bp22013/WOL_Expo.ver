...
import { Monitor, Globe, Hash, Trash2 } from 'lucide-react-native';
import Animated, { FadeIn } from 'react-native-reanimated';

export default function EditDeviceScreen() {
    const { id } = useLocalSearchParams<{ id: string }>();
    const { devices, updateDevice, deleteDevice, isLoading } = useDevices();
    ...
    const handleUpdateDevice = async () => {
        ...
        try {
            ...
            setError('デバイスを更新できませんでした。 再度お試しください');
        }
    };

    const handleDeleteDevice = () => {
        Alert.alert('デバイスを削除', 'デバイスを本当に削除しますか？', [
            { text: 'キャンセル', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        setError('デバイスを削除できませんでした。 再度お試しください');
                    }
                },
            },
        ]);
            ...
            <ScrollView style={styles.scrollView}>
                <Animated.View entering={FadeIn.duration(400)} style={styles.formContainer}>
                    ...
                    <TouchableOpacity
                        ...
                        disabled={isLoading}
                    >
                        <Trash2 size={20} color="#E53935" style={styles.deleteIcon} />
